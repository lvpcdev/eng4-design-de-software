 ...
 * Arquivo: OperationsObserver.java
 *
 * Finalidade:
...
public class OperationsObserver implements DeliveryObserver {
    public void onStatusChanged(Delivery delivery) {
        System.out.println("[OPERAÇÃO] entrega " + delivery.shipment().id() + " -> " + delivery.status());
    }
}
